...
class AE(nn.Module):
    ...
    def __init__(self,network='default', nz=8 , num_layers=18):
       ...
       # Convolution layer to reduce channels from 64 to 32
        self.enc_conv = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1)
       ...
       # Fully connected layer to expand 8-dim vector to 32 * 8 * 8
        ...
        self.dec_deconv1 = nn.ConvTranspose2d(in_channels=32, out_channels=64, kernel_size=4, stride=2, padding=1)
        self.dec_bn1 = nn.BatchNorm2d(64)
        
        # Transposed conv layer to go from 64x16x16 to 64x32x32
        self.dec_deconv2 = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=4, stride=2, padding=1)
